# تطوع بدر - التطوع المدرسي الذكي (Smart School Volunteering)
...
عند أول تشغيل، سكربت الزراعة (`npm run seed`) ينشئ حساب مسؤول نظام افتراضي
واحد فقط (`admin@bayan.edu.sa` / `password123`) إذا لم يوجد أي حساب بدور
"مسؤول النظام" مسبقاً. غيّر البريد وكلمة المرور فوراً من صفحة "حسابي" بعد
أول دخول. لا يوجد حسابات تجريبية أخرى — أنشئ حسابات المعلمين والطلاب من
لوحة "إدارة المستخدمين" أو عبر التسجيل الذاتي للطلاب.
